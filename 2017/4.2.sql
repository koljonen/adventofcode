CREATE FUNCTION Normalize(text) returns text language sql AS
$$
SELECT string_agg(Letter, '' ORDER BY Letter)
FROM (
    SELECT substr($1, Idx, 1) AS Letter
    FROM generate_series(1, length($1)) Idx
) X;
$$;
...
SELECT SUM(((SELECT COUNT(DISTINCT Normalize(Word)) FROM unnest(Split) Word) = (SELECT COUNT(*) FROM unnest(Split) Word))::int)
FROM Split;
