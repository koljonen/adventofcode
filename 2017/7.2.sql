...
CREATE UNLOGGED TABLE Programs(Name text PRIMARY KEY, Weight int);
CREATE UNLOGGED TABLE Connections(Child text PRIMARY KEY REFERENCES Programs, Parent text REFERENCES Programs);
...
kligtj (80)')
    ...
    SELECT (regexp_matches(Split[1], '\w+'))[1] AS Program, (regexp_matches(Split[1], '\((\d+)\)'))[1] AS Weight, string_to_array(Split[2], ', ') AS Children
    FROM Split
), InsertPrograms AS (
    INSERT INTO Programs(Name, Weight)
    SELECT Program, Weight::int
    FROM Progs
)
INSERT INTO Connections(Parent, Child)
SELECT Program, unnest(Children)
FROM Progs;

CREATE FUNCTION GetWeight(_Program text) RETURNS int LANGUAGE SQL AS $$
SELECT Weight + COALESCE(
    (
        SELECT SUM(GetWeight(Child))
        FROM Connections
        WHERE Parent = $1
    ),
    0
...
WITH ProblemParent AS (
    SELECT Parent
    FROM Connections
    GROUP BY Parent
    HAVING COUNT(DISTINCT GetWeight(Child)) > 1
    ORDER BY GetWeight(Parent) LIMIT 1
), Majority AS (
    SELECT GetWeight(Child) AS TotalWeight
    FROM Connections
    JOIN ProblemParent USING (Parent)
    GROUP BY 1
    HAVING COUNT(*) > 1
), Minority AS (
    SELECT GetWeight(Child) AS TotalWeight, MAX(Programs.Weight) AS OwnWeight
    FROM Connections
    JOIN ProblemParent USING (Parent)
    JOIN Programs ON Programs.Name = Connections.Child
    GROUP BY 1
    HAVING COUNT(*) = 1
)
SELECT Minority.OwnWeight - Minority.TotalWeight + Majority.TotalWeight
