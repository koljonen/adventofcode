...
CREATE TABLE Registers(Register text PRIMARY KEY, Val int NOT NULL DEFAULT 0);

CREATE TABLE Instructions AS
WITH Input(Input) AS (VALUES('d dec 683 if qn == 0
d dec -220 if h == 0
...
h inc -526 if d != -2461')
    ...
    SELECT regexp_split_to_array(regexp_split_to_table, ' ') AS Split, Ordinality
    FROM Input
    CROSS JOIN LATERAL regexp_split_to_table(Input, '\n') WITH ORDINALITY
)
SELECT Split[1] AS Register, (Split[2] = 'inc')::int * 2 - 1 AS Sign, Split[3]::int AS Val, Split[5] AS LReg, Split[6] AS Op, Split[7]::int AS RVal, Ordinality
FROM Split;

CREATE FUNCTION Execute(_Instruction Instructions) RETURNS int LANGUAGE SQL AS $$
INSERT INTO Registers(Register) SELECT $1.Register
ON CONFLICT (Register) DO NOTHING;
UPDATE Registers SET Val = Val + $1.Sign * $1.Val
WHERE Register = $1.Register
AND CASE $1.op
    WHEN '==' THEN COALESCE((SELECT L.Val FROM Registers L WHERE L.Register = $1.LReg), 0) = $1.RVal
    WHEN '!=' THEN COALESCE((SELECT L.Val FROM Registers L WHERE L.Register = $1.LReg), 0) != $1.RVal
    WHEN '<=' THEN COALESCE((SELECT L.Val FROM Registers L WHERE L.Register = $1.LReg), 0) <= $1.RVal
    WHEN '>=' THEN COALESCE((SELECT L.Val FROM Registers L WHERE L.Register = $1.LReg), 0) >= $1.RVal
    WHEN '<'  THEN COALESCE((SELECT L.Val FROM Registers L WHERE L.Register = $1.LReg), 0) <  $1.RVal
    WHEN '>'  THEN COALESCE((SELECT L.Val FROM Registers L WHERE L.Register = $1.LReg), 0) >  $1.RVal
END;
SELECT MAX(Val) FROM Registers;
$$;

WITH RECURSIVE Execution AS (
    SELECT Execute.*, Ordinality + 1 AS Ordinality
    FROM Instructions
    CROSS JOIN LATERAL Execute(Instructions)
    WHERE Ordinality = 1
    UNION ALL
    SELECT Execute.* , Ordinality + 1
    FROM Execution
    JOIN Instructions USING(Ordinality)
    CROSS JOIN LATERAL Execute(Instructions)
)
SELECT MAX(Execute)
FROM Execution;
