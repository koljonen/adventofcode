...
lines = [n for n in input.split('\n')]
parsed = [re.match(r'(\w+) \((\d+)\)( -> ((\w+,? ?)+))?', line) for line in lines]
programs = dict()
connections = dict()
for p in parsed:
    programs[p.group(1)] = int(p.group(2))
    if p.group(4):
        connections[p.group(1)] = p.group(4).split(', ')

def get_weight(program):
    return programs[program] + sum(get_weight(child) for child in connections.get(program, []))

imbalanced = [p for p, cs in connections.items() if len(set(get_weight(c) for c in cs)) > 1]
bottom_imbalance = min(imbalanced, key=get_weight)
child_weights = defaultdict(list)
for c in connections[min(imbalanced, key=get_weight)]:
    child_weights[get_weight(c)].append(c)
minority = [cs[0] for w, cs in child_weights.items() if len(cs) == 1][0]
majority = [cs[0] for w, cs in child_weights.items() if len(cs) != 1][0]
print(programs[minority] + get_weight(majority) - get_weight(minority))
